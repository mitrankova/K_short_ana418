...
int KFParticle_sPHENIX::InitRun(PHCompositeNode *topNode)
{
  ...
  getField();
  std::cout<<"KFParticle_sPHENIX::InitRun - Got field!"<<std::endl;
  return 0;
}
...
void KFParticle_sPHENIX::getField()
{
  ...
  std::cout<<"KFParticle_sPHENIX::getField - Got fieldmap from magfile!"<<std::endl;
 ...
 // float z = 1.;
  ...
  while (Bz == 0)
  {
  std::cout<<"KFParticle_sPHENIX::getField - In while Bz==0"<<std::endl;
    if (n == 4)
    {
      ++r;
    }
  ...
  std::cout<<"KFParticle_sPHENIX::getField - n = "<<n<<" r = "<<r<<" arc = "<<arc<<std::endl;

    fieldmap->Draw(">>elist", sweep.c_str(), "entrylist");
  std::cout<<"KFParticle_sPHENIX::getField - sweep = "<<sweep<<std::endl;
    TEntryList *elist = (TEntryList*)gDirectory->Get("elist");
    std::cout<<"KFParticle_sPHENIX::getField - Got elist"<<std::endl;
    if (elist->GetEntry(0))
    {
      TLeaf *xpos = fieldmap->GetLeaf("bz");
      xpos->GetBranch()->GetEntry(elist->GetEntry(0));
      Bz = xpos->GetValue();
  std::cout<<"KFParticle_sPHENIX::getField - Bz get value"<<std::endl;
    }
  ...
  std::cout<<"KFParticle_sPHENIX::getField - Out of while"<<std::endl;
